...
def detect_port_scan():
    ...
    while True:
        ...
        remove_list = []
        for _, packet_cap in packet_dictionary.items():
...
def deconstruct_packet(packet_captured):
    try:
        ...
        if ("TCP" in packet_captured):
            ...
            if ( source_ip not in packet_dictionary ):
            # Create a new key in the dictionary
                packet_dictionary[source_ip] = compiled_packet
                dictionary_lock.release()
                 
            else:
            # Add to the start of a dictionary linked list
                compiled_packet.tailLength = 1 + packet_dictionary[source_ip].tailLength    # Increase head's leangth by one from the last 
...
if __name__ == "__main__":
    ...
    # Dictionaries for packets and unsafe packets
    packet_dictionary = { }
    unsafe_packet_dictionary = { }
